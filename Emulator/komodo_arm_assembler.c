...
// SJH Disabling emission of optimsed LDR as a MOV or MVN for 1-1 mapping of assembler
// with PC-relative load.
                ...
                if (line[position] == '=')
                  ...
                  if ((token & 0xFFF00000) != 0x02400000)     /* Only for LDR */
                    ...
                    if ((error_code == eval_okay)
                      || allow_error(error_code, first_pass, last_pass))
                      {
			      // SJH: Hardcoded switch to always emit LDR. removed switch on do_literal
                      switch (2)
				      //do_literal(THUMB, TYPE_WORD, &value,
...
/*============================================================================*/
/*Changes by Simon Hollis to original aasm.c source:

LDR instruction: 
1) Hard-coded switch to always emit PC-relative LDR an never
optimise via MOV or MOVN.

2) Disabled literal pool addition on a PC-relative LDR to support simple
PC-relative loads, based on labels.*/